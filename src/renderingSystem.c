...
void renderRenderingSystem(Entity_t *ent){
    if(ent->components.textureComponent->texture&&ent->components.transformComponent&&ent->components.rectComponent){
        SDL_Rect rect;
        rect.x = ent->components.transformComponent->position.x + ent->components.rectComponent->x;
        rect.y = ent->components.transformComponent->position.y + ent->components.rectComponent->y;
